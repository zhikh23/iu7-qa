...
- [Программирование. Подготовка к экзамену](#программирование-подготовка-к-экзамену)
  ...
  - [Теоретические вопросы](#теоретические-вопросы)
    ...
    - [12. Операторы цикла. Цикл с условием. Операторы break и continue. Примеры использования](#12-операторы-цикла-цикл-с-условием-операторы-break-и-continue-примеры-использования)
      ...
      - [Операторы цикла](#операторы-цикла-в-python)
    ...
    - [13. Операторы цикла. Цикл с итератором. Функция range(). Примеры использования](#13-операторы-цикла-цикл-с-итератором-функция-range-примеры-использования)
      ...
      - [Операторы цикла](#операторы-цикла-в-python-1)
    ...
    - [59. Списки. Сортировка. Обменные методы сортировки. Сортировка пузырьком. Сортировка пузырьком с флагом. Метод шейкер-сортировки](#59-списки-сортировка-обменные-методы-сортировки-сортировка-пузырьком-сортировка-пузырьком-с-флагом-метод-шейкер-сортировки)
      ...
      - [Сортировка кучей](#сортировка-кучей-пирамидальная)
...
_Numpy_ — математический модуль для языка Python. Главной вещью, которую он добавляет, являются многомерные массивы, а так же математические операции в виде пре-компилированных пакетов, что занчительно ускоряет их.

> Комментарий: _Numpy_ — очень большой и сложный модуль, в связи с этим, в статье будут рассказаны только основные моменты, которые,
> по мнению автора, могут быть на экзамене. Читателю рекомендуется, как минимум, перейти по ссылкам, приведённым в статье, и ознакомиться
> с документацией модуля.

Начнём с подключения модуля, он не находится в стандартном _venv_, в связи с чем его сначала требуется установить (например с помощью _pip_), и только затем его можно импортировать.
...
# С указанеием типа содержимого
ar = np.array([1, 2, 3], float)
print(ar)
#Out: [1. 2. 3.]

# Можно и двумерный, да и вообще любой натуральной мерности
...
2. С помощью `arange`. <br>
`arange` — работает как `range`, но допускает действительные значения начала, конца и шага
```py
ar = np.arange(0.1, 0.3, 0.05)
print(ar)
#Out: [0.1 0.15 0.2 0.25]
```

3. C помошью `linespace`. <br>
...
```py
import numpy as np
ar = np.linspace(1, 3, 5)
print(ar)
#Out: [1. 1.5 2. 2.5 3.]
```
4. Специализированными функциями. <br>
В numpy существует ещё множество специализированных функций для создания различных массивов, ссылка на них ниже: 

> Подробнее почитать о созданиии массивов можно [тут](https://numpy.org/doc/stable/reference/routines.array-creation.html#)

Действия с массивами

Срезы и обращение у элементу для массивов работают так же как и для списков, за исключеним того, что если в списках для каждой новой мерности открывались новые квадратные скобки, в массивах всё перечисляется в одних скобках через запятую. Покажу на примере:

```py
#Список
l = [[1, 2, 3], [4, 5, 6]]
print(l[0][1]])  # Две пары скобок
# Out: 2

#Массив
ar = np.array([[1, 2, 3], [4, 5, 6]])
print(ar[0,1])  # Одна пара скобок, перечисление через запятую
# Out: 2
```
...
- `len(a)` — возвращает длину первого измерения массива
...
- `reshape(a, newshape\[, order])` — изменяет мерность массива
- `transpose(a\[, axes])` — возвращает новый массив, полученный транспонированием предыдущего
...
- `unique(ar\[, return_index, return_inverse, ...])` — возвращает массив уникальных элементов другого массива
- `flip(m\[, axis])` — отражает массив до указанной оси
- `roll(a, shift\[, axis])` — циклически сдвигает элементы массива вдоль указанной оси
...
Доступны арифметические операции между массивами и между массивом и списком, если размерность мерности по которой проводится операция у операндов совпадает. Все операции проводятся поэлментно с соответствующими элментами. Пример:
...
# А вот умножение ar на [1, 2] выдаст ошибку т.к. у массива длина внешнего измерения 3, а у списка 2
```
...
- `dot(a, b)`  — произведение, как произведение матриц

> Комментарий: Нетрудно заметить, что с массивами можно делать всё то же, что и со списками. Более того,
> чаще всего _Python_ сам сможет привести типы и операция корректно проведётс между списком и массивом.

#### Работа с числами

С точки зрения работы с числами в _numpy_ есть все (ну или почти все) функции, что и в модуле _math_ но лучше.
Они обобщены (по возможности) на множество вещественных чисел, а также быстрее и точнее, чем в _math_. Отчасти это достигается 
за счёт новых типов данных, по сути взятых из _C_ и полностью совместимых с типами данных _C_.
